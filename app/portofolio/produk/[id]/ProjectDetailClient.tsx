      ...
      <div className="container mx-auto px-6">
        ...
        >
          <Link
            ...
            style={{ color: "#5a8aaf" }}
            onMouseEnter={e => (e.currentTarget as HTMLAnchorElement).style.color = "#3a6a8f"}
            onMouseLeave={e => (e.currentTarget as HTMLAnchorElement).style.color = "#5a8aaf"}
          >
            <motion.svg
              className="w-5 h-5"
              fill="none"
        ...
        >
          ...
          >
            {[
              ...
              <motion.div
                key={info.label}
                className="bg-black/5 backdrop-blur-sm border border-black/10 rounded-2xl p-6"
                variants={staggerItem}
                whileHover={{ y: -4, borderColor: "rgba(255,255,255,0.2)" }}
                transition={{ type: "spring", stiffness: 300, damping: 20 }}
              >
                <h3 className="text-sm text-zinc-500 mb-2">{info.label}</h3>
                <p className={`font-semibold ${info.value ? "text-gray-900" : "text-zinc-500 italic"}`}>
                  {info.value || "—"}
                </p>
              </motion.div>
            ))}
        ...
        <div className="max-w-4xl mx-auto mb-20">
          <div className="space-y-16">
            ...
            {/* Overview Section */}
            <div ref={overviewRef}>
              <h2 className="text-3xl font-bold text-gray-900 mb-6">Tentang Project</h2>
              <div className="space-y-4">
                <p className="text-lg text-zinc-600 leading-relaxed">{project.description}</p>
                {project.overview && (
            ...
            <div ref={techRef}>
              <h2 className="text-3xl font-bold text-gray-900 mb-6">Teknologi yang Dipakai</h2>
              {project.tools.length > 0 ? (
                ...
                >
                  {project.tools.map((tech) => (
                    <motion.span
                      key={tech}
                      className="px-4 py-2 bg-black/5 border border-black/10 rounded-full text-sm text-zinc-600 cursor-default"
                      variants={staggerItem}
                      whileHover={{ scale: 1.07, borderColor: "rgba(188,224,255,0.9)", color: "#2a5a8a", backgroundColor: "rgba(188,224,255,0.15)" }}
                      transition={{ type: "spring", stiffness: 400, damping: 17 }}
                    >
                      {tech}
                    </motion.span>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {project.features.map((feature, i) => (
                    <motion.div
                      ...
                      whileHover={{ y: -5, borderColor: "rgba(255,200,221,0.6)" }}
                    >
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-4 font-bold text-sm" style={{ backgroundColor: "#FFC8DD", color: "#7a2a42" }}>
                        {i + 1}
                      </div>
